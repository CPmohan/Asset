...
func HandleAssetUpload(w http.ResponseWriter, r *http.Request) {
	...
	for _, group := range payload.Group {
		_, err := tx.Exec("INSERT INTO asset_groups (asset_id, group_name) VALUES (?, ?)", assetID, strings.TrimSpace(group))
		if err != nil {
			tx.Rollback()
...
func HandleGetAsset(w http.ResponseWriter, r *http.Request) {
	...
	err := db.QueryRow(`
		...
		FROM assets a
		JOIN asset_groups ag ON a.id = ag.asset_id
		WHERE a.name = ? AND LOWER(TRIM(ag.group_name)) = ?`, assetName, strings.ToLower(strings.TrimSpace(groupName))).
		Scan(&assetID, &asset.Name, &asset.Quantity)
	if err != nil {
	...
	for rows.Next() {
		...
		// Add the full URL path to the file as a new field fileUrl
		...
		files = append(files, FileInfo{
			...
			// Add fileUrl as a new field in the struct (need to update struct)
		})
	}
	asset.Files = files
