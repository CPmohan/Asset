  ...
  useEffect(() => {
    if (state) {
      ...
      axios
        ...
        .then((res) => {
          console.log("API Response:", res.data); // Add this to inspect the response
          setAsset(res.data);
        })
        .catch((err) => {
    ...
    return (
      <ul className="list-disc list-inside text-sm">
        {asset.files.map((f, index) => {
          ...
          // const fileName = f.name || f.fileName || f.file?.name || "File";
          // const fileType = f.type || "Unknown";
          // const fileUrl = f.url || f.fileUrl || f.path;
          const fileName = f.filename || "Unnamed File";
          const fileType = f.type || "Unknown";
          const fileUrl = f.filename?.startsWith("/uploads/")
            ? `http://localhost:8080${f.filename}`
            : f.filename;
