...
// This creates an object like { electronics: 2, transport: 1, ... }
const groupCounts = {};
assetData.forEach((item) => {
  const groupKey = item.group.trim().toLowerCase(); // Normalize group name
    ...
    <div style={{ padding: "1rem" }}>
      ...
      title="ALL ASSET GROUPS"
        ...
        data={groups.map((group) => {
          const groupKey = group.trim().toLowerCase(); // Normalize for lookup
          return {
            group,
            ...
            onClick: () =>
              navigate(`/group/${group}`, {
                state: {
                  assetData: assetData.filter((a) => a.group === group),
                },
              }),
          };
        })}
