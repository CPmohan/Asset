...
export const getAsset = async (groupName, assetName) => {
  ...
  return {
    ...data,
    group: data.Group && data.Group.length > 0 ? data.Group[0] : '',
    files: data.Files ? data.Files.map(file => ({
      filename: file.Filename,        // Used in frontend
      type: file.Type,                // Used in frontend
      typeValue: file.TypeValue       // Optional - if used
    })) : []
  };
};
...
export const getAssetsByGroup = async (groupName) => {
  ...
  return data.assets.map(asset => ({
    ...asset,
    group: asset.Group && asset.Group.length > 0 ? asset.Group[0] : ''
  }));
};
